...
CREATE TABLE IF NOT EXISTS `db-theme-3`.`adminAnswer` (
  ...
  INDEX `fk_SupportRequest_Export_idx` (`History_History.id` ASC) VISIBLE,
     CONSTRAINT `fk_SupportRequest_Export`
        FOREIGN KEY (`History_History.id`)
        REFERENCES `db-theme-3b`.`History` (`History.id`)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION))
ENGINE = InnoDB;
-- -----------------------------------------------------
...
START TRANSACTION;
USE `db-theme-3`;
INSERT INTO `db-theme-3`.`Export` (`Export.id`, `Export.isSuccess`, Export.time) VALUES (1, 'True', 2024-3-26);
INSERT INTO `db-theme-3`.`Export` (`Export.id`, `Export.isSuccess`, Export.time) VALUES (2, 'True', 2024-4-17);
INSERT INTO `db-theme-3`.`Export` (`Export.id`, `Export.isSuccess`, Export.time) VALUES (3, 'False', 2024-4-23);
